...
How do you even begin? What can you do to find the underlying structure of this dataset? How might you summarize it? What are the subgroups within the data? Are there outliers? How could you effectively represent the data visually? Machine learning has the answers!


## Supervised vs Unsupervised Machine Learning

In short, answers to these questions boil down to whether or not your data is labeled.  If you’re lucky, you already know about the structure of your data. Congratulations! You can now analyze your data with supervised machine learning techniques. With “supervised” learning, the algorithm can be taught what the correct output is for given inputs. The algorithm will then search for patterns in the data corresponding to this information, and can then label new input data. Supervised machine learning algorithms include many of the most famous statistical analysis methods, such as linear regression and k-Nearest Neighbors. These methods have been used extensively, and are incredibly useful to science because their performance can be judged by a variety of metrics, like accuracy and precision.
...
K-Means requires the parameter *k* , which sets the number of clusters in the dataset. The algorithm will then:
...
These last three steps repeat until the algorithm converges, as exemplified in the above gif. 

Despite its efficacy and ease of use, K-Means has several significant disadvantages. Firstly, the user must find the optimal number of clusters (*k*). K-Means is very sensitive to this parameter and will be rendered effectively useless without it's optimization. Second, K-Means iterates repeatedly until all clusters are equal in size no matter the distribution of the data. Finally, K-Means doesn’t consider the density of datapoints, and therefore does not recognize outliers. For these reasons, K-Means is unsuited to discovering clusters that are not ellipsoid in shape.

DBSCAN (Density-Based Spatial Clustering of Applications with Noise) attempts to solve some of the shortcomings of K-Means by clustering datapoints based on density, effectively ignoring “sparse” sections of data by labelling them as noise. This allows the algorithm to work robustly with “noisy” datasets, both for identifying clusters within them and for efficiently identifying outliers. DBSCAN’s focus on density instead of mean distance results in efficient modeling of non-ellipsoid structures in the data because it allows clusters to take an irregular shape, which is often more representative of organic data. 
...
Comparison of K-Means (above) vs DBSCAN (below) on two different datasets: note that the K-Means does not recognize outliers or properly represent non-ellipsoid shaped clusters.
<p align="right">
...
DBSCAN implementation depends on two parameters to determine sample density.<sup name="a4">[4](#f4)</sup> First, **ε, epsilon** (abbreviated to "eps"), the radius from any datapoint used to calculate each point’s neighbors. The simplest and most commonplace technique used is euclidian distance (right). 

Second, a natural number, **"min_samples"** (often abbreviated to "minPts"), the minimum number of datapoints within the epsilon neighborhood from a single datapoint. This value serves as the threshold for how many points must be around a “core point” in order for the neighborhood to be considered a cluster. Generally, a min_samples value of ≤ 3 is not productive. Larger values work better for larger datasets, and so min_samples should scale somewhat with the size of the data. Too large of a min_sample value will result in an overly smooth density estimate. The scientist typically uses their domain knowledge to estimate what a good starting min_sample value for the dataset may be. 


Additionally, when describing DBSCAN clusters, several terms are important:
- “core point”: point (*p*) is defined as a core point if at least min_samples points are within the epsilon neighborhood N(p) = { p ∈ Data | dist(p, q) ≤ ε} of it (including p itself).
...
- determine which points are neighbors of *p* using euclidian distance calculated using ε,
- create a new cluster around *p* and its neighbors if it is a "core point" (N(p) ≥ min_samples),
- visit the next point in the dataset, marking it as a border point (ε ≤ N(p)) or noise (ε ≥ N(p)).
